...
import { Box, Container, FormControl, FormLabel, Heading, Input, Stack, Text, useToast, Select as Select2, Checkbox, Flex, Divider, Modal, ModalOverlay, ModalContent, ModalHeader, ModalCloseButton, ModalBody, ModalFooter, useDisclosure, TableContainer, Table, Thead, Tr, Th, Tbody, Td, Link, IconButton } from "@chakra-ui/react"
...
export default function Registration({ params: { id } }: RegistrationEdit) {
  ...
  const naturalAllDays = [{ label: 'Segunda', value: 1 },
  ...
  const handleReturn = async () => { router.push("/registration") }

  const handleRenovate = async () => {
    api.post(`/registration/${id}/renovate`, {
        ...
        <Box display="flex" flexDirection={{ base: 'column', md: 'row' }}>
          <Box flex="1" mr={{ md: 4 }}>
            ...
            <FormControl mb={4}>
              ...
              {!(id > 0) &&
                <Select placeholder='Selecione o Aluno' onInputChange={handleLoadStudent} options={studentSelect}
                  onChange={(e: any) => { setRegistration({ ...registration, student_id: e.value }) }} />}
              {id > 0 &&
                <Input colorScheme={'blackAlpha'} isReadOnly value={registration.student.name} />}
            </FormControl>
            <FormControl mb={4}>
              <FormLabel htmlFor="activity">Atividade</FormLabel>
              <Select2 placeholder='Selecione a Atividade' value={registration.activity_id} onChange={e => { setRegistration({ ...registration, activity_id: +e.target.value }) }}>
            ...
            <FormControl mb={4}>
              ...
              <Select2 placeholder='Selecione o professor' value={registration.professional_id} onChange={e => { setRegistration({ ...registration, professional_id: +e.target.value }) }}>
            ...
            <Flex justifyContent={'space-between'}>
              <FormControl mb={4} mr="3%">
                ...
                <Select isMulti placeholder='Selecione os Dias' options={naturalAllDays}
                  value={allDays}
                  onChange={(e: any) => {
                    setRegistration({ ...registration, day_of_week: e.map((item: any) => item.value) })
                    setAllDays(naturalAllDays.filter(item => e.map((aux: any) => aux.value).includes(item.value)))
                  }} />
              </FormControl>
              <FormControl mb={4} maxW={'12%'} >
                <FormLabel htmlFor="status">Status</FormLabel>
                <Checkbox id="status" name="status" placeholder="Status" isChecked={registration.status}
                  borderColor={'black'}
                  onChange={e => { setRegistration({ ...registration, status: !registration.status }) }}></Checkbox>
              </FormControl>
            </Flex>
            <Flex justifyContent={'space-between'}>
              <FormControl mb={4} mr="3%">
                <FormLabel htmlFor="start_time">Hora Início</FormLabel>
                <Input type="time" id="start_time" name="start_time" placeholder="Hora Início"
                  value={registration?.start_time}
                  onChange={e => { setRegistration({ ...registration, start_time: e.target.value }) }}
                />
              </FormControl>
              <FormControl mb={4}>
                <FormLabel htmlFor="end_time">Hora Fim</FormLabel>
                <Input type="time" id="end_time" name="end_time" placeholder="Hora fim"
                  value={registration?.end_time}
                  onChange={e => { setRegistration({ ...registration, end_time: e.target.value }) }}
                />
              </FormControl>
            </Flex>
            <FormControl mb={4}>
              <FormLabel htmlFor="expiration">Data de Expiração</FormLabel>
              <Input type="date" id="expiration" name="expiration" placeholder="Data de Expiração" value={registration?.expiration}
                onChange={e => { setRegistration({ ...registration, expiration: e.target.value }) }} />
            </FormControl>
                      ...
                      <Tbody>
                        {purchases && purchases?.map((item: any) => (
                          <Tr key={item.id}>
                            ...
                            <Td>{item.status == 'pending' ? 'Pendente' : item.status == 'paid' ? 'Pago' : 'Cancelado'}</Td>
                            <Td>{formatDate(new Date(item.creation_date))}</Td>
                            <Td>
                              <Link href={`/purchase/${item.id}`}><IconButton
                                variant="outline"
                                textAlign={'right'}
                                aria-label="Link"
                                icon={<TbExternalLink />}
        ...
        <ModalContent>
          ...
          <ModalBody>
            <FormControl mb={4}>
              ...
              <Select2 placeholder='Selecione o Tipo' value={purchase.payment_type} onChange={e => { setPurchase({ ...purchase, payment_type: e.target.value }) }}>
            ...
            <FormControl mb={4}>
              ...
              <Input id="value" name="value" type="number" placeholder="Valor" value={purchase?.value}
            ...
            {id > 0 &&
              <FormControl mb={4}>
                <FormLabel htmlFor="expiration">Data de Expiração</FormLabel>
                <Input type="date" id="expiration" name="expiration" placeholder="Data de Expiração" value={purchase?.expiration}
                  onChange={e => { setPurchase({ ...purchase, expiration: e.target.value }) }} />
              </FormControl>}
          </ModalBody>
          <ModalFooter>
