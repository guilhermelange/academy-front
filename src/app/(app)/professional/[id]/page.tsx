...
export default function Professional({ params: { id } }: ProfessionalEdit) {
  ...
  const handleReturn = async () => { router.push("/professional") }

  const handleUpdate = async () => {
    api.put(`/professional/${id}`, {
      ...
      <Box display="flex" flexDirection={{ base: 'column', md: 'row' }}>
        <Box flex="1" mr={{ md: 4 }}>
          ...
          <FormControl mb={4}>
            ...
            <Input id="name" name="name" placeholder="Nome" value={professional?.name}
              onChange={e => { setProfessional({ ...professional, name: e.target.value }) }} />
          </FormControl>
          <FormControl mb={4}>
            <FormLabel htmlFor="birth_date">Data de Nascimento</FormLabel>
            <Input type="date" id="birth_date" name="birth_date" placeholder="Data Nascimento" value={professional?.birth_date}
              onChange={e => { setProfessional({ ...professional, birth_date: e.target.value }) }} />
          </FormControl>
          <FormControl mb={8}>
            <FormLabel htmlFor="academic_education">Formação</FormLabel>
            <Input id="academic_education" name="academic_education" placeholder="Formação" value={professional?.academic_education}
              onChange={e => { setProfessional({ ...professional, academic_education: e.target.value }) }} />
          </FormControl>
          <FormControl mb={4}>
            <Stack direction={'row'}>
